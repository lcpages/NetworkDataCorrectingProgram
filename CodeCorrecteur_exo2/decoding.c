...
int thereIsError(CodeWord_t *cw, int data_size)
{
	...
	for(int k=0; k < data_size; k++)
	{
		...
		//On récupére le degré du poly gPoly puis on monte la puissances des
		// monomes  
		degree = getDegree(gPoly); 
		gPoly = gPoly<<(getDegree(cw[k])-getDegree(gPoly)); 
		left = cw[k]; 
		int n_exit =0;
		//On effectue la divison sur le mot actuel 
		while(getDegree(left) > degree && n_exit <16){ 
				//	printBits(gPoly, "gPoly"); 
					left = xor(left, gPoly); 
			//		printBits(left, "left"); 
					gPoly = gPoly>>(getDegree(gPoly)-getDegree(left));
					n_exit++;
		}
	//	Si le reste n'est pas vide, alors il y a une erreure 
		if(left != 0) //
		{
			 errorDetected =1; 
